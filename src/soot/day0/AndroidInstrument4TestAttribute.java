...
public class AndroidInstrument4TestAttribute {
				...
				if ("<purainli.first.FullscreenActivity: void test()>".equals(method.getSignature()))
				{
					...
					for(Iterator<Unit> iter = units.snapshotIterator(); iter.hasNext();) 
					{
						...
						if (stmt instanceof ReturnStmt || stmt instanceof ReturnVoidStmt)
						{
							...
							AddExpr addExpr = Jimple.v().newAddExpr(Jimple.v().newStaticFieldRef(
									Scene.v().getField("<purainli.first.FullscreenActivity: long counter>").makeRef()), LongConstant.v(1));
							units.insertBefore(Jimple.v().newAssignStmt(Jimple.v().newStaticFieldRef(
									Scene.v().getField("<purainli.first.FullscreenActivity: long counter>").makeRef()), addExpr), stmt);
					        ...
					        StaticInvokeExpr staticInvokeExpr = Jimple.v().newStaticInvokeExpr(valueOf.makeRef(), 
					        		Jimple.v().newStaticFieldRef(
									Scene.v().getField("<purainli.first.FullscreenActivity: long counter>").makeRef()));
